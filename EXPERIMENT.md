...
- **발언 턴은 도구를 가진다.**
  - 항상 허용: 웹 검색, 그리고 방의 워크스페이스·artifacts 폴더의 읽기/검색.
  - 사람의 명시적 요청이 있을 때만: 파일 생성·수정과 코드 실행. 이 쓰기는
    **세션 전용 artifacts 폴더 안으로 기계적으로 격리**된다 — 규칙 문장이 아니라
    실행 환경(도구 구성)이 보장하며, 규칙은 그 위의 자제 의무만 진다.
